...
using clinicbot.Dialogs.CreateAppoinment;
using clinicbot.Dialogs.Qualificacion;
using clinicbot.Dialogs.TestCovid;
...
namespace clinicbot.Dialogs
{
    public class RootDialog: ComponentDialog
    {
        ...
        private async Task<DialogTurnResult> ManageIntentions(WaterfallStepContext stepContext, RecognizerResult luisResult, CancellationToken cancellationToken)
        {
            ...
            {
                case "CasosCovid":
                    await CasosCovid(stepContext, cancellationToken);
                    break;
                case "TestCovid":
                    return await TestCovid(stepContext, luisResult, cancellationToken);
                case "Saludar":
        ...
        private async Task CasosCovid(WaterfallStepContext stepContext, CancellationToken cancellationToken)
        {
            var messageUser = stepContext.Context.Activity.Text.ToLower();
            var resultCovidData = await _covid19CountryService.Execute("Peru");
            if (messageUser.Contains("casos") || messageUser.Contains("infectados"))
            {
                string message = $"Esta es la información que tengo:" +
                   $"{Environment.NewLine}Total casos: {resultCovidData.cases.ToString()}";
                await stepContext.Context.SendActivityAsync(message, cancellationToken: cancellationToken);
            }
            else if (messageUser.Contains("recuperado"))
            {
                string message = $"Esta es la información que tengo:" +
                    $"{Environment.NewLine}Recuperados: {resultCovidData.recovered.ToString()}";
                await stepContext.Context.SendActivityAsync(message, cancellationToken: cancellationToken);
            }
            else if (messageUser.Contains("fallecido") || messageUser.Contains("muertes") || messageUser.Contains("muertos") || messageUser.Contains("muertes"))
            {
                string message = $"Esta es la información que tengo:" +
                    $"{Environment.NewLine}Fallecidos: {resultCovidData.deaths.ToString()}";
                await stepContext.Context.SendActivityAsync(message, cancellationToken: cancellationToken);
            }            
            else
            {
                string message = $"📍 CASOS CONFIRMADOS EN PERÚ" +                    
                    $"{Environment.NewLine}Total casos: {resultCovidData.cases.ToString()}" +
                    $"{Environment.NewLine}Recuperados: {resultCovidData.recovered.ToString()}" +
                    $"{Environment.NewLine}Fallecidos: {resultCovidData.deaths.ToString()}"+
                $"{Environment.NewLine}Activos Covid: {resultCovidData.active.ToString()}";

                await stepContext.Context.SendActivityAsync(message, cancellationToken: cancellationToken);
            }
        }

        private async Task<DialogTurnResult> TestCovid(WaterfallStepContext stepContext, RecognizerResult luisResult, CancellationToken cancellationToken)
        {
            return await stepContext.BeginDialogAsync(nameof(TestCovidDialog), cancellationToken: cancellationToken);
        }
        ...
        {
            var resultQnA = await _qnAMakerAIServive._qnaMakerResult.GetAnswersAsync(stepContext.Context);
     ...
     + "   ¿Por qué ? Las autoridades locales y nacionales son los interlocutores más indicados para dar consejos sobre lo que deben hacer las personas de su zona para protegerse.", cancellationToken: cancellationToken);
        ...
        private async Task IntentVerCita(WaterfallStepContext stepContext, RecognizerResult luisResult, CancellationToken cancellationToken)
        {
            ...
            if(medicalData.Count > 0)
            {
                ...
                if (pending.Count > 0)
                {
                    await stepContext.Context.SendActivityAsync("Estas son tus citas pendiente", cancellationToken: cancellationToken);
        ...
        {
            return await stepContext.BeginDialogAsync(nameof(CreateAppoinmentDialog), cancellationToken: cancellationToken);
        }
